...
def pop_str_args(request_handler, argument_name, regex_string=None, default=no_default):
    ...
    for input in inputs:
        assert isinstance(input, basestring), "input \"%s\" must be a string" % input
        if regex_string:
...
def pop_int_arg(request_handler, argument_name, default=None, **kwargs):
    if default == no_default:
        inputs = pop_num_args(request_handler, argument_name, default=no_default, **kwargs)
    else:
        inputs = pop_num_args(request_handler, argument_name, default=[None], **kwargs)
